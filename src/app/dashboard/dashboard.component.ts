...
export class DashboardComponent implements OnInit {
  ...
  constructor(public authService: AuthService, private firestore: AngularFirestore) {
    ...
    if (this.authService.currentUser.displayName.toLowerCase().includes("cody") || this.authService.currentUser.displayName.toLowerCase().includes("eric")) {
      this.menuItems.push({
        text: "Expense",
        number: 5
      });
  ...
  addExpense() {
    ...
    this.showProps.showFinanceBeer = false;
    this.showProps.showAddCase = false;
    this.showProps.showExpenses = true;
  }
